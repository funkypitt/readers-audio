package com.freedomfighter.readersaudio.audio
...
/**
 * Decode any audio file MediaCodec reads and hand it over as mono 16 kHz pieces of about
 * [seconds] each, with where each piece starts. Keeps memory flat whatever the length: an
 * audiobook of ten hours never sits in memory whole. Stops as soon as [onChunk] returns false.
 */
object Decode16k {
    fun chunks(ctx: Context, uri: Uri, seconds: Int, onChunk: (FloatArray, Long) -> Boolean) {
        ...
        fun flush() {
            ...
            val out = Resample.to(Pcm(acc.toArray(), rate), 16_000).samples
            acc = FloatList()
            keepGoing = onChunk(out, startMs)
            startMs += ms
        }
        try {
            val info = MediaCodec.BufferInfo()

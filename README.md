...
* **Transcribe**: whisper.cpp runs on the phone (model base, small or medium, fetched once),
  the file decoded in five-minute pieces so a long audiobook never sits in memory whole.
  The transcript is a plain `.txt` in **Documents/Transcriptions**, which any reader opens —
  Reader's Books included. Language: the phone's, English, or detected.
* **Share** the audio or the transcript to any app, and **save a copy to a folder…** straight
